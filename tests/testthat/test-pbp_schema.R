...
testthat::test_that('gc_play_by_play() returns the public schema and fills goal shooters', {
  gc_raw <- data.frame(
    eventId = 1:3,
  ...
  testthat::local_mocked_bindings(
    .perform_parallel_requests = function(reqs, on_error = 'return') {
      testthat::expect_named(reqs, c('pbp_meta', 'html_pbp'))
      list(
  ...
  testthat::expect_true(all(c('playerId', 'blockingPlayerId', 'servedByPlayerId') %in% names(out)))
  testthat::expect_true(all(is.na(out$playerId)))
  testthat::expect_true(all(is.na(out$blockingPlayerId)))
  testthat::expect_true(all(is.na(out$servedByPlayerId)))
  testthat::expect_equal(out$goalieInNetId, c(NA_integer_, 30L, 40L))
  testthat::expect_false('homeGoalieSecondsElapsedInShift' %in% names(out))
  testthat::expect_false('homeGoalieSecondsElapsedInPeriodSinceLastShift' %in% names(out))
  testthat::expect_equal(
...
# Run pbp schema tests.
testthat::test_that('wsc_play_by_play() returns the public schema with utc and no clip fields', {
  wsc_raw <- data.frame(
    id = 999L,
    eventId = 1:3,
    period = c(1L, 1L, 1L),
  ...
  testthat::local_mocked_bindings(
    .perform_parallel_requests = function(reqs, on_error = 'return') {
      testthat::expect_named(reqs, c('pbp_meta', 'wsc', 'html_pbp'))
      list(
        pbp_meta = structure(list(kind = 'pbp_meta'), class = 'mock_resp'),
  ...
  testthat::expect_match(names(out)[10], '^utc$')
  strength_idx <- match('strengthState', names(out))
  testthat::expect_equal(
    names(out)[(strength_idx + 1L):(strength_idx + 4L)],
    c(
...
# Run pbp schema tests.
testthat::test_that('raw play-by-play helpers return flattened raw source rows', {
  gc_raw <- data.frame(
    eventId = 1:2,
    sortOrder = 1:2,
  ...
  testthat::local_mocked_bindings(
    .nhl_api = function(path, type, ...) {
      testthat::expect_identical(type, 'w')
  ...
  testthat::expect_equal(names(gc_out)[1], 'gameId')
  testthat::expect_equal(gc_out$gameId, rep(2010020001L, 2L))
  testthat::expect_true('periodDescriptor.number' %in% names(gc_out))
  testthat::expect_equal(names(wsc_out)[1], 'gameId')
  testthat::expect_equal(wsc_out$gameId, rep(2010020001L, 2L))
  testthat::expect_false('id' %in% names(wsc_out))
})

# Run pbp schema tests.
testthat::test_that('repair_public_pbp_sequence drops invalid clocks and repairs boundary order', {
  pbp <- data.frame(
  ...
  testthat::expect_false(13L %in% out$eventId)
  testthat::expect_equal(out$eventTypeDescKey, c('period-start', 'faceoff', 'hit', 'shot-on-goal', 'period-end'))
  testthat::expect_equal(out$timeInPeriod[out$eventTypeDescKey == 'period-end'], '20:00')
  testthat::expect_true(all(diff(out$secondsElapsedInPeriod) >= 0))
})

# Run pbp schema tests.
testthat::test_that('illogically ordered boundary faceoffs are removed', {
  pbp <- data.frame(
    gameId = rep(1L, 5L),
  ...
  testthat::expect_false(12L %in% out$eventId)
  testthat::expect_equal(out$eventId, c(10L, 11L, 13L, 14L))
})
